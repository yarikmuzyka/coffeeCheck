...
function coffeePayload(formData) {
  ...
  const weightGrams = int(formData.get('weightGrams'))
  const pricePer100g =
    price != null && weightGrams ? Math.round((price / weightGrams) * 1000) / 10 : null
  return {
    name: str(formData.get('name')),
    roasterId: str(formData.get('roasterId')),
    originCountry: str(formData.get('originCountry')),
    region: str(formData.get('region')),
    farm: str(formData.get('farm')),
    producer: str(formData.get('producer')),
    variety: str(formData.get('variety')),
    process: str(formData.get('process')),
    roastLevel: str(formData.get('roastLevel')),
    roastDate: date(formData.get('roastDate')),
    purchaseDate: date(formData.get('purchaseDate')),
    price,
    weightGrams,
    pricePer100g,
    declaredNotes: str(formData.get('declaredNotes')),
    userNotes: str(formData.get('userNotes')),
    productUrl: str(formData.get('productUrl')),
...
export async function deleteBrewLog(formData) {
  const id = str(formData.get('id'))
  const coffeeId = str(formData.get('coffeeId'))
  if (id) {
